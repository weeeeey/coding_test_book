...
[계수 정렬]
특정 조건에서 사용하는 정렬
데이터 개수 N, 최댓값이 K 일 경우 O(N+K)
데이터의 크기 범위가 제한되어 (((((정수 형태)))))로 표현될떄만 사용
데이터 차이가 1백만 이하일떄 효과적 -> 모든 범위를 담을 수 있는 크기의 리스트를 선언 해야 하기 때문
비교 기반 알고리즘 x
1. max와 min을 담을 하나의 리스트를 만든다.
2. 0으로 초기화
3. 데이터를 하나씩 확인하며 동일한 인덱스의 데이터를 1씩 증가시키면 계수 정렬 완료
4. 리스트 자체가 정렬된 상태니까 해당 값의 인덱스 값만큼 출력하면 끝
'''

arr = [ 7,7,3,1,7,9,10,3300 ]
result = [0]*(max(arr)+1)
for i in range(len(arr)):
  result[arr[i]] += 1
